...
class Login extends Component {
  ...
  componentDidMount() {
    let token = Cookie.get('panzer_access_token');
    if (token) {
      ...
      .then(response => {
        ...
        if (response && response.status == 200) {
          ...
          if (process.env.NODE_ENV === 'production') {
            Cookie.set('panzer_access_token', `${token}`, { expires: 14 })
          }
          else {
            Cookie.set('panzer_access_token', `${token}`, { expires: 14 })
          }
                ...
                <Card className="text-white bg-roomy-blue py-5 d-md-down-none" style={{ width: 44 + '%' }}>
                  <CardBody className="text-center">
                    ...
                    <div style={style.logoWrapper} className={`svg-logo`}>
                      ...
                      <div className={`text-center`} style={{ fontSize: '20px', paddingTop: "10px", fontWeight: "bold" }}>Panzer Dashboard</div>
                    </div>
                  </CardBody>
                </Card>
              </CardGroup>
